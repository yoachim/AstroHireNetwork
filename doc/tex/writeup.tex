...
\section{TO DO}
\begin{itemize}
  \item{re-run the data scrape to account for mixed case names. DONE-Maybe also strip accents, etc?--note use of leven to match author names in cases where accents may have been converted in some journals. }
\end{itemize}
...
For common names (e.g.,  Williams, B), our author query returns entries from multiple authors.  To try and restrict the results to only those papers with the same author as the PhD thesis, we construct a network graph using \citet{networkx}.
...
\begin{figure}
  \plottwo{../../plots/example_network_5.pdf}{../../plots/example_network_6.pdf}
...
\begin{figure}
  ...
  \plotone{../../plots/linkCheck_errorbars.pdf}
